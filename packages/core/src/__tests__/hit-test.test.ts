...
describe('dispatchHit', () => {
  ...
  it('scroll-adjusted child origin overflow skips descendants on Y (symmetric with scrollX + large child x)', () => {
    const max = Number.MAX_VALUE
    let childFired = false
    const child = box({
      y: max,
    ...
    const layout = {
      ...
      width: 100,
      height: 300,
      children: [{ x: 0, y: max, width: 50, height: 50, children: [] as const }],
    }
    expect(() => dispatchHit(parent, layout, 'onClick', 25, max)).not.toThrow()
    expect(dispatchHit(parent, layout, 'onClick', 25, max)).toEqual({ handled: false })
    expect(childFired).toBe(false)
    expect(hitPathAtPoint(parent, layout, 25, max)).toEqual([])
    expect(hasInteractiveHitAtPoint(parent, layout, 25, max)).toBe(false)
    expect(getCursorAtPoint(parent, layout, 25, max)).toBeNull()
  })
  ...
  it('overlapping siblings: three distinct z-index values prefer the highest (multi-child sort path)', () => {
    const log: string[] = []
  ...
  it('overlapping siblings: equal z-index on two children prefers later source order (two-child fast path)', () => {
    const log: string[] = []
  ...
  it('overlapping siblings: three children with partial z-index ties still prefer highest z (general sort path, not identical-z fast path)', () => {
    ...
    const top = box({ width: 50, height: 50, zIndex: 2, onClick: () => { log.push('top') } })
    const root = box({ width: 100, height: 100 }, [lowA, lowB, top])
    const layout = {
      x: 0,
      y: 0,
      width: 100,
  ...
  it('overlapping siblings: shrinking from three to two children recomputes z-order (cache length mismatch)', () => {
    const log: string[] = []
    const low = box({ width: 50, height: 50, zIndex: 0, onClick: () => log.push('low') })
    const mid = box({ width: 50, height: 50, zIndex: 5, onClick: () => log.push('mid') })
    const high = box({ width: 50, height: 50, zIndex: 10, onClick: () => log.push('high') })
    ...
    dispatchHit(root, layout3, 'onClick', 10, 10)
    expect(log).toEqual(['high'])
    ...
    const layout2 = {
      ...layout3,
      children: layout3.children.slice(0, 2),
    }
    dispatchHit(root, layout2, 'onClick', 10, 10)
    expect(log).toEqual(['mid'])
    expect(hitPathAtPoint(root, layout2, 10, 10)).toEqual([1])
...
describe('scroll and overflow clipping', () => {
  ...
  it('overflow scroll: MAX_VALUE origin plus -MAX_VALUE scrollY overflows child offset; descendants are not hit', () => {
    let childFired = false
    const child = box({
      width: 50,
      height: 50,
    ...
    const max = Number.MAX_VALUE
    const parent = box({ width: 100, height: 100, overflow: 'scroll', scrollY: -max }, [child])
    const layout = {
      x: 0,
      y: max,
      width: 100,
    ...
    expect(max - -max).toBe(Infinity)
    dispatchHit(parent, layout, 'onClick', 50, max)
    expect(childFired).toBe(false)
...
describe('z-index sort cache invalidation', () => {
  it('recomputes sibling order when a child is appended (cached z-order length mismatch)', () => {
    const log: string[] = []
    const back = box({ width: 50, height: 50, zIndex: 0, onClick: () => { log.push('back') } })
    const root = box({ width: 100, height: 100 }, [back])
    const layout = {
      x: 0,
      y: 0,
      width: 100,
  ...
  it('recomputes sibling order when a two-child parent mutates zIndex in place (two-child fast-path cache zValues)', () => {
    const log: string[] = []
    const back = box({ width: 50, height: 50, zIndex: 0, onClick: () => { log.push('back') } })
    const front = box({ width: 50, height: 50, zIndex: 10, onClick: () => { log.push('front') } })
    const root = box({ width: 100, height: 100 }, [back, front])
  ...
  it('recomputes sibling order when shrinking three children to two by removing the middle (stale multi-child cache)', () => {
    ...
    const mid = box({ width: 50, height: 50, zIndex: 5, onClick: () => { log.push('mid') } })
    const front = box({ width: 50, height: 50, zIndex: 10, onClick: () => { log.push('front') } })
    const root = box({ width: 100, height: 100 }, [back, mid, front])
    const layout = {
      x: 0,
      y: 0,
      width: 100,
  ...
  it('recomputes sibling order when a three-child parent mutates zIndex in place (multi-child cache zValues)', () => {
    const log: string[] = []
    const back = box({ width: 50, height: 50, zIndex: 0, onClick: () => { log.push('back') } })
    const mid = box({ width: 50, height: 50, zIndex: 5, onClick: () => { log.push('mid') } })
    const front = box({ width: 50, height: 50, zIndex: 10, onClick: () => { log.push('front') } })
  ...
  it('recomputes sibling order when a child is removed (cached z-order length mismatch)', () => {
    const log: string[] = []
    const back = box({ width: 50, height: 50, zIndex: 0, onClick: () => { log.push('back') } })
    const front = box({ width: 50, height: 50, zIndex: 10, onClick: () => { log.push('front') } })
    const root = box({ width: 100, height: 100 }, [back, front])
  ...
  it('recomputes z-order when shrinking from three children to two after multi-child cache warm', () => {
    const log: string[] = []
    const low = box({ width: 50, height: 50, zIndex: 0, onClick: () => { log.push('low') } })
    const mid = box({ width: 50, height: 50, zIndex: 5, onClick: () => { log.push('mid') } })
    const high = box({ width: 50, height: 50, zIndex: 10, onClick: () => { log.push('high') } })
  ...
  it('recomputes sibling order when growing from two children to three after two-child cache warm', () => {
    const log: string[] = []
    const back = box({ width: 50, height: 50, zIndex: 0, onClick: () => { log.push('back') } })
    const front = box({ width: 50, height: 50, zIndex: 10, onClick: () => { log.push('front') } })
    const root = box({ width: 100, height: 100 }, [back, front])
    ...
    const top = box({ width: 50, height: 50, zIndex: 20, onClick: () => { log.push('top') } })
    root.children.push(top)
    layout.children.push({ x: 0, y: 0, width: 50, height: 50, children: [] as const })
...
describe('Yoga-computed row layout and hit routing (document direction)', () => {
  ...
  it('LTR document: nested flex row with dir rtl mirrors children; dispatchHit and hit queries follow mirrored geometry', () => {
    const log: string[] = []
    ...
    const innerRow = box(
      { width: 200, height: 50, flexDirection: 'row', dir: 'rtl' },
      [first, second],
    )
  ...
  it('LTR document: nested flex row with dir auto inherits owner direction; dispatchHit follows non-mirrored geometry', () => {
    const log: string[] = []
    const first = box({ width: 50, height: 50, cursor: 'pointer', onClick: () => { log.push('dom-first') } })
    const second = box({ width: 50, height: 50, cursor: 'pointer', onClick: () => { log.push('dom-second') } })
    const innerRow = box(
