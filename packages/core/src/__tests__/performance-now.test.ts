...
describe('clampNonNegativeLayoutWallMs', () => {
  ...
  it('rejects non-number and non-finite values without throwing (typeof + Number.isFinite; parity with layout-bounds)', () => {
    ...
    expect(clampNonNegativeLayoutWallMs(true as unknown as number)).toBe(0)
    expect(clampNonNegativeLayoutWallMs(false as unknown as number)).toBe(0)
    expect(() => clampNonNegativeLayoutWallMs(Symbol('x') as unknown as number)).not.toThrow()
    expect(clampNonNegativeLayoutWallMs(Symbol('x') as unknown as number)).toBe(0)
