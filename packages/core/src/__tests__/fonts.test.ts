...
describe('collectFontFamiliesFromTree', () => {
  ...
  it('walks nested boxes in preorder, skipping scene3d leaves between text nodes', () => {
    const tree = box({}, [
      text({ text: '1', font: '12px FirstFamily, serif', lineHeight: 16 }),
      box({}, [
        scene3d({ objects: [], width: 1, height: 1 }),
        text({ text: '2', font: '12px SecondFamily, serif', lineHeight: 16 }),
      ]),
      text({ text: '3', font: '12px ThirdFamily, serif', lineHeight: 16 }),
    ])
...
describe('waitForFonts', () => {
  ...
  it('no-ops when document.fonts is null', async () => {
    vi.stubGlobal('document', { fonts: null })
    await expect(waitForFonts(['Inter'])).resolves.toBeUndefined()
  })

  it('no-ops when document.fonts.load is not a function', async () => {
    vi.stubGlobal('document', { fonts: { load: true as never, ready: Promise.resolve() } })
    await expect(waitForFonts(['Inter'])).resolves.toBeUndefined()
  })
