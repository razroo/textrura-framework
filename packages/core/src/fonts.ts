 ...
 * If `defaultMs` is also invalid, falls back to `10_000`.
...
export function resolveFontLoadTimeoutMs(timeoutMs: number | undefined, defaultMs = 10_000): number {
  const safeDefault =
    typeof defaultMs === 'number' && Number.isFinite(defaultMs) && defaultMs >= 0 ? Math.max(0, defaultMs) : 10_000
 ...
 * Uses `document.fonts.load` per family when it is a function; otherwise a no-op (avoids corrupt stubs).
 * Timeouts are swallowed so startup never hard-fails.
...
export async function waitForFonts(families: string[], timeoutMs = 10_000): Promise<void> {
  ...
  if (api == null || typeof api.load !== 'function') return
