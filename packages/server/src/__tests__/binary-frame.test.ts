...
describe('isBinaryFrameBuffer', () => {
  ...
  it('returns false when byteLength / byteOffset accessors throw (exotic Proxy; probe stays non-throwing)', () => {
    ...
    expect(() => isBinaryFrameBuffer(proxyBuf)).not.toThrow()
    ...
    expect(() => isBinaryFrameBuffer(proxyView)).not.toThrow()
    expect(isBinaryFrameBuffer(proxyView)).toBe(false)
  ...
  it('returns false for plain objects or fake views with non-finite byte layout (undefined < 9 is not a length check)', () => {
    ...
    expect(
      isBinaryFrameBuffer({ buffer: ab, byteOffset: 0, byteLength: undefined } as unknown as Uint8Array),
    ).toBe(false)
    expect(
      isBinaryFrameBuffer({ buffer: ab, byteOffset: undefined, byteLength: 16 } as unknown as Uint8Array),
    ).toBe(false)
    expect(
      isBinaryFrameBuffer({ buffer: ab, byteOffset: 0, byteLength: Number.NaN } as unknown as Uint8Array),
    ).toBe(false)
    expect(
      isBinaryFrameBuffer({ buffer: ab, byteOffset: 0, byteLength: Number.POSITIVE_INFINITY } as unknown as Uint8Array),
    ).toBe(false)
    expect(
      isBinaryFrameBuffer({ buffer: ab, byteOffset: -1, byteLength: 16 } as unknown as Uint8Array),
    ).toBe(false)
    expect(isBinaryFrameBuffer({ buffer: null, byteOffset: 0, byteLength: 20 } as unknown as Uint8Array)).toBe(false)
  })

  it('returns false when fake view byteLength exceeds backing buffer (no RangeError from Uint8Array ctor)', () => {
    const ab = new ArrayBuffer(8)
    const fake = { buffer: ab, byteOffset: 0, byteLength: 100 } as unknown as Uint8Array
    expect(isBinaryFrameBuffer(fake)).toBe(false)
    expect(() => decodeBinaryFrameJson(fake)).toThrow('Not a GEOM binary frame')
  })

  it('returns false when fake view byteOffset + byteLength overflows the backing store', () => {
    const ab = new ArrayBuffer(16)
    expect(
      isBinaryFrameBuffer({ buffer: ab, byteOffset: 10, byteLength: 10 } as unknown as Uint8Array),
    ).toBe(false)
  })
