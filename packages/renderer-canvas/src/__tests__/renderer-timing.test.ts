...
describe('CanvasRenderer sibling zIndex paint order', () => {
  it('paints overlapping equal finite z-index siblings in stable source order (last child on top; matches hit-test tie-break)', () => {
    ...
    const tree = box({ width: 100, height: 100, backgroundColor: '#ffffff' }, [
      box({ width: 100, height: 100, backgroundColor: '#ff0000', zIndex: 0 }),
      box({ width: 100, height: 100, backgroundColor: '#0000ff', zIndex: 0 }),
    ])
    const layout = {
      x: 0,
      y: 0,
