...
describe('createClient WebSocket message parse errors', () => {
  ...
  it('invokes onError when frame root layout omits x or y (undefined after JSON parse), then accepts a valid frame', async () => {
    const sockets: Array<{ emit(type: string, event?: unknown): void }> = []
    installMockWebSocket(sockets)
    ...
    sockets[0]!.emit('message', {
      data: JSON.stringify({
        type: 'frame',
        layout: { y: 0, width: 10, height: 10, children: [] },
        tree: { kind: 'box', props: {}, children: [] },
        protocolVersion: 1,
      }),
    })
    ...
    sockets[0]!.emit('message', {
      data: JSON.stringify({
        type: 'frame',
        layout: { x: 0, width: 10, height: 10, children: [] },
        tree: { kind: 'box', props: {}, children: [] },
        protocolVersion: 1,
      }),
    })
    ...
    const validFrame = {
      type: 'frame',
      layout: { x: 0, y: 0, width: 41, height: 42, children: [] },
      tree: { kind: 'box', props: {}, children: [] } satisfies UIElement,
      protocolVersion: 1,
    }
    sockets[0]!.emit('message', { data: JSON.stringify(validFrame) })
    await new Promise<void>(resolve => queueMicrotask(() => resolve()))
    expect(renders).toHaveLength(1)
    expect(renders[0]?.width).toBe(41)
    expect(client.layout?.width).toBe(41)
  })
  ...
  it('invokes onError when frame root layout has negative subnormal width/height (layoutBoundsAreFinite), then accepts a valid frame', async () => {
    const sockets: Array<{ emit(type: string, event?: unknown): void }> = []
    installMockWebSocket(sockets)
    ...
    sockets[0]!.emit('message', {
      data: JSON.stringify({
        type: 'frame',
        layout: { x: 0, y: 0, width: negSub, height: 10, children: [] },
        tree: { kind: 'box', props: {}, children: [] },
        protocolVersion: 1,
      }),
    })
    ...
    sockets[0]!.emit('message', {
      data: JSON.stringify({
        type: 'frame',
        layout: { x: 0, y: 0, width: 10, height: negSub, children: [] },
        tree: { kind: 'box', props: {}, children: [] },
        protocolVersion: 1,
      }),
    })
    ...
    const validFrame = {
      type: 'frame',
      layout: { x: 0, y: 0, width: 51, height: 52, children: [] },
      tree: { kind: 'box', props: {}, children: [] } satisfies UIElement,
      protocolVersion: 1,
    }
    sockets[0]!.emit('message', { data: JSON.stringify(validFrame) })
    await new Promise<void>(resolve => queueMicrotask(() => resolve()))
    expect(renders).toHaveLength(1)
    expect(renders[0]?.width).toBe(51)
    expect(client.layout?.width).toBe(51)
  })
  ...
  it('invokes onError when frame root layout has non-finite x or y (Infinity), then accepts a valid frame', async () => {
    const sockets: Array<{ emit(type: string, event?: unknown): void }> = []
    installMockWebSocket(sockets)
    ...
    sockets[0]!.emit('message', {
      data: JSON.stringify({
        type: 'frame',
        layout: { x: Number.POSITIVE_INFINITY, y: 0, width: 10, height: 10, children: [] },
        tree: { kind: 'box', props: {}, children: [] },
        protocolVersion: 1,
      }),
    })
    ...
    sockets[0]!.emit('message', {
      data: JSON.stringify({
        type: 'frame',
        layout: { x: 0, y: Number.NEGATIVE_INFINITY, width: 10, height: 10, children: [] },
        tree: { kind: 'box', props: {}, children: [] },
        protocolVersion: 1,
      }),
    })
    ...
    const validFrame = {
      type: 'frame',
      layout: { x: 0, y: 0, width: 59, height: 60, children: [] },
      tree: { kind: 'box', props: {}, children: [] } satisfies UIElement,
      protocolVersion: 1,
    }
    sockets[0]!.emit('message', { data: JSON.stringify(validFrame) })
    await new Promise<void>(resolve => queueMicrotask(() => resolve()))
    expect(renders).toHaveLength(1)
    expect(renders[0]?.width).toBe(59)
    expect(client.layout?.width).toBe(59)
  })
  ...
  it('invokes onError when frame root layout has array or object bounds (JSON type confusion), then accepts a valid frame', async () => {
    const sockets: Array<{ emit(type: string, event?: unknown): void }> = []
    installMockWebSocket(sockets)
    ...
    sockets[0]!.emit('message', {
      data: JSON.stringify({
        type: 'frame',
        layout: { x: 0, y: 0, width: 10, height: { v: 1 }, children: [] },
        tree: { kind: 'box', props: {}, children: [] },
        protocolVersion: 1,
      }),
    })
    ...
    const validFrame = {
      type: 'frame',
      layout: { x: 0, y: 0, width: 53, height: 54, children: [] },
      tree: { kind: 'box', props: {}, children: [] } satisfies UIElement,
      protocolVersion: 1,
    }
    sockets[0]!.emit('message', { data: JSON.stringify(validFrame) })
    await new Promise<void>(resolve => queueMicrotask(() => resolve()))
    expect(renders).toHaveLength(1)
    expect(renders[0]?.width).toBe(53)
    expect(client.layout?.width).toBe(53)
  })
  ...
  it('accepts JSON frame when protocolVersion is omitted (no version guard on the wire)', async () => {
    const sockets: Array<{ emit(type: string, event?: unknown): void }> = []
    installMockWebSocket(sockets)
    ...
    sockets[0]!.emit('message', {
      data: JSON.stringify({
        type: 'frame',
        layout: { x: 0, y: 0, width: 81, height: 82, children: [] },
        tree: { kind: 'box', props: {}, children: [] },
      }),
    })
    await new Promise<void>(resolve => queueMicrotask(() => resolve()))
    ...
    expect(errors).toHaveLength(0)
    expect(renders).toHaveLength(1)
    expect(renders[0]?.width).toBe(81)
  ...
  it('invokes onError when frame protocolVersion is JSON null (e.g. NaN serialized), then accepts a valid frame', async () => {
    const sockets: Array<{ emit(type: string, event?: unknown): void }> = []
    installMockWebSocket(sockets)
    ...
    const validFrame = {
      type: 'frame',
      layout: { x: 0, y: 0, width: 47, height: 48, children: [] },
      tree: { kind: 'box', props: {}, children: [] } satisfies UIElement,
      protocolVersion: 1,
    }
    sockets[0]!.emit('message', { data: JSON.stringify(validFrame) })
  ...
  it('invokes onError when frame protocolVersion is a JSON string (type confusion), then accepts a valid frame', async () => {
    ...
    const validFrame = {
      type: 'frame',
      layout: { x: 0, y: 0, width: 46, height: 45, children: [] },
      tree: { kind: 'box', props: {}, children: [] } satisfies UIElement,
      protocolVersion: 1,
    }
    sockets[0]!.emit('message', { data: JSON.stringify(validFrame) })
    await new Promise<void>(resolve => queueMicrotask(() => resolve()))
    expect(renders).toHaveLength(1)
    expect(renders[0]?.width).toBe(46)
  ...
  it('invokes onError when frame protocolVersion is non-finite (JSON exponent overflow to Infinity), then accepts a valid frame', async () => {
    ...
    sockets[0]!.emit('message', {
      data: '{"type":"frame","layout":{"x":0,"y":0,"width":1,"height":1,"children":[]},"tree":{"kind":"box","props":{},"children":[]},"protocolVersion":1e400}',
    })
    await new Promise<void>(resolve => queueMicrotask(() => resolve()))
    expect(errors).toHaveLength(1)
    expect(errors[0]).toBeInstanceOf(Error)
    ...
    const validFrame = {
      type: 'frame',
      layout: { x: 0, y: 0, width: 45, height: 44, children: [] },
      tree: { kind: 'box', props: {}, children: [] } satisfies UIElement,
      protocolVersion: 1,
    }
    sockets[0]!.emit('message', { data: JSON.stringify(validFrame) })
    await new Promise<void>(resolve => queueMicrotask(() => resolve()))
    expect(renders).toHaveLength(1)
    expect(renders[0]?.width).toBe(45)
    expect(client.layout?.width).toBe(45)
  })
  ...
  it('invokes onError when frame root layout omits children (undefined after JSON parse), then accepts a valid frame', async () => {
    const sockets: Array<{ emit(type: string, event?: unknown): void }> = []
    installMockWebSocket(sockets)
    ...
    sockets[0]!.emit('message', {
      data: '{"type":"frame","layout":{"x":0,"y":0,"width":1,"height":1},"tree":{"kind":"box","props":{},"children":[]},"protocolVersion":1}',
    })
    await new Promise<void>(resolve => queueMicrotask(() => resolve()))
    expect(errors).toHaveLength(1)
    expect(errors[0]).toBeInstanceOf(Error)
    ...
    const validFrame = {
      type: 'frame',
      layout: { x: 0, y: 0, width: 46, height: 47, children: [] },
      tree: { kind: 'box', props: {}, children: [] } satisfies UIElement,
      protocolVersion: 1,
    }
    sockets[0]!.emit('message', { data: JSON.stringify(validFrame) })
  ...
  it('invokes onError when type is frame but layout is null (JSON null is not a layout object), then accepts a valid frame', async () => {
    const sockets: Array<{ emit(type: string, event?: unknown): void }> = []
    installMockWebSocket(sockets)
    ...
    expect(errors).toHaveLength(1)
    expect(errors[0]).toBeInstanceOf(Error)
    expect(String((errors[0] as Error).message)).toMatch(/Invalid server message/)
    expect(renders).toHaveLength(0)
    expect(client.layout).toBeNull()
  ...
  it('invokes onError when type is frame but tree is null (JSON null is not a plain object), then accepts a valid frame', async () => {
    const sockets: Array<{ emit(type: string, event?: unknown): void }> = []
    installMockWebSocket(sockets)
    ...
    const validFrame = {
      type: 'frame',
      layout: { x: 0, y: 0, width: 49, height: 50, children: [] },
      tree: { kind: 'box', props: {}, children: [] } satisfies UIElement,
      protocolVersion: 1,
    }
    sockets[0]!.emit('message', { data: JSON.stringify(validFrame) })
    await new Promise<void>(resolve => queueMicrotask(() => resolve()))
    expect(renders).toHaveLength(1)
    expect(renders[0]?.width).toBe(49)
    expect(client.layout?.width).toBe(49)
  })
  ...
  it('decodes GEOM v1 binary frames when event.data is Float16Array (2-byte element view)', async () => {
    ...
    const frame = encodeGeomV1JsonPayload(
      JSON.stringify({
        type: 'frame',
        layout: { x: 0, y: 0, width: 67, height: 68, children: [] },
        tree: { kind: 'box', props: {}, children: [] },
        protocolVersion: 1,
      }),
    )
    const alignedLen = Math.ceil(frame.byteLength / 2) * 2
    const aligned = new ArrayBuffer(alignedLen)
    new Uint8Array(aligned).set(new Uint8Array(frame))

    sockets[0]!.emit('message', { data: new Float16Array(aligned) })
    await new Promise<void>(resolve => queueMicrotask(() => resolve()))
    expect(errors).toHaveLength(0)
    expect(renders).toHaveLength(1)
    expect(renders[0]?.width).toBe(67)
  })
  ...
  it('invokes onError when data message omits payload (undefined after JSON parse), then accepts a valid frame', async () => {
    const sockets: Array<{ emit(type: string, event?: unknown): void }> = []
    installMockWebSocket(sockets)
    ...
    const validFrame = {
      type: 'frame',
      layout: { x: 0, y: 0, width: 70, height: 71, children: [] },
      tree: { kind: 'box', props: {}, children: [] } satisfies UIElement,
      protocolVersion: 1,
    }
    sockets[0]!.emit('message', { data: JSON.stringify(validFrame) })
    await new Promise<void>(resolve => queueMicrotask(() => resolve()))
    expect(renders).toHaveLength(1)
    expect(renders[0]?.width).toBe(70)
    expect(client.layout?.width).toBe(70)
  })

  it('delivers well-formed data messages to onData over WebSocket (metrics, no render)', async () => {
    const sockets: Array<{ emit(type: string, event?: unknown): void }> = []
    installMockWebSocket(sockets)
    ...
    const payload = { n: 42, nested: { ok: true } }
    sockets[0]!.emit('message', {
      data: JSON.stringify({
        type: 'data',
        channel: 'geom.notify',
    ...
    expect(errors).toHaveLength(0)
    expect(renders).toHaveLength(0)
    expect(dataCalls).toEqual([{ channel: 'geom.notify', payload }])
    expect(metrics).toHaveLength(1)
