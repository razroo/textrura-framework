...
describe('isBinaryFrameArrayBuffer', () => {
  ...
  it('returns false when fake view byteLength exceeds backing buffer (no RangeError from Uint8Array ctor)', () => {
    ...
    expect(isBinaryFrameArrayBuffer(fake)).toBe(false)
  ...
  it('returns false when fake view byteOffset + byteLength overflows the backing store', () => {
    const ab = new ArrayBuffer(16)
    expect(
      isBinaryFrameArrayBuffer({ buffer: ab, byteOffset: 10, byteLength: 10 } as unknown as Uint8Array),
    ).toBe(false)
  })
  ...
  it('returns false for non-integer byteLength on fake views (Uint8Array ctor would throw)', () => {
    const ab = new ArrayBuffer(32)
    expect(
      isBinaryFrameArrayBuffer({ buffer: ab, byteOffset: 0, byteLength: 9.5 } as unknown as Uint8Array),
    ).toBe(false)
  })
  ...
  it('returns false when magic or version on a root SharedArrayBuffer does not match v1', () => {
    ...
    expect(isBinaryFrameArrayBuffer(sab)).toBe(false)
    ...
    expect(isBinaryFrameArrayBuffer(wrongVersion)).toBe(false)
  })
...
describe('client binary frame decode', () => {
  ...
  it('decodes a v1 frame when the same bytes are exposed as Float32Array, Float64Array, or Int8Array', () => {
    ...
    expect(isBinaryFrameArrayBuffer(f32)).toBe(true)
  ...
  it('decodes a v1 frame when the same bytes are exposed as Float16Array (ArrayBuffer.isView parity)', () => {
    ...
    expect(isBinaryFrameArrayBuffer(f16)).toBe(true)
