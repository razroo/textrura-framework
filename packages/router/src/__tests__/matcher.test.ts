...
describe('route matcher', () => {
  it('returns null for non-string pattern or pathname without throwing (parity with parseQuery guards)', () => {
    ...
    expect(() => matchPath(1n as never, '/a')).not.toThrow()
    expect(matchPath(1n as never, '/a')).toBeNull()
    expect(() => matchPath('/a', 2n as never)).not.toThrow()
    expect(matchPath('/a', 2n as never)).toBeNull()
