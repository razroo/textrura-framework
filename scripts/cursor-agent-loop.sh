...
# The same config **excludes** slow or heavy suites via `test.exclude` in `vitest.fast.config.ts` — read
# that file for the authoritative list (it may grow; do not rely on examples in this comment alone).
# `npm run test` omits those paths even when `release:gate` runs them explicitly. Do not treat a green
# fast test run as a substitute for `release:gate` when editing any excluded surface.
...
# `geometry-snapshot-ci.test.ts`, `layout-bounds.test.ts`, `hit-test.test.ts`, `keyboard.test.ts`, `text-input.test.ts`,
# `text-input-invariants.test.ts`, `text-input-history.test.ts`, `virtual-scroll.test.ts`, `performance-now.test.ts`,
# and `app-layout-timing.test.ts`
# (geometry CI, layout coordinate invariants, hit routing, keyboard dispatch / focus order, text input + IME/history,
# virtual list windowing, monotonic timing / hostile `performance` guards, createApp + `Renderer.setFrameTimings` layout
# telemetry — see `requiredVitestAllowlistPaths` in verify-release-gate.mjs).
...
#   when your edits touch those surfaces (demos, `create:app` / starter templates, examples tooling, Playwright demo
#   flows, MCP benchmarks). When editing MCP benchmark scripts or harness expectations, run that benchmark locally
#   (or the relevant `benchmark:mcp-*` variant), not only `release:gate`.
...
2. Decide what to work on. Use this priority order:
   ...
   b) If nothing is unchecked there either, improve the framework on your own initiative. Examples of valuable work:
      ...
      When adding tests without a specific bugfix, prefer extending files already run by root \`package.json\` \`release:gate\` (keeps new coverage in the vetted CI path; only widen the gate when the suite is release-critical). The gate is an explicit file allowlist. \`npm run test\` uses \`vitest.fast.config.ts\`, which **excludes** some \`__tests__\` files via \`test.exclude\` — read that file for the list; those suites still run in \`release:gate\` and \`test:all\`. Root \`npm run test:all\` / \`bun run test:all\` runs vitest.fast then explicitly adds fonts, virtual-scroll, and perf-smoke suites — broader than \`npm run test\`, but still not equivalent to \`release:gate\` (different argv + terminal input). If you widen the gate, grep or read the existing \`vitest run ...\` list first so you do not add a duplicate path (vitest would execute that file twice). Paths are not grouped by package: e.g. \`layout-bounds.test.ts\` may already be the first argv path — search the whole line, not only near related tests. \`scripts/release/verify-release-gate.mjs\` (\`requiredVitestAllowlistPaths\`) must include \`geometry-snapshot-ci.test.ts\`, \`layout-bounds.test.ts\`, \`hit-test.test.ts\`, \`keyboard.test.ts\`, \`text-input.test.ts\`, \`text-input-invariants.test.ts\`, \`text-input-history.test.ts\`, \`virtual-scroll.test.ts\`, \`performance-now.test.ts\`, and \`app-layout-timing.test.ts\` — do not drop these when trimming \`release:gate\` (keep in sync with the verifier source).
